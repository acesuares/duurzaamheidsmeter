...
// the maps in html
	...
	$map_html .= "<TABLE WIDTH=\"100%\" BORDER=\"0\">\n";
		$map_html .= "<TR>\n";
			...
			$map_html .= "<INPUT TYPE=\"image\" NAME=\"kaart\" SRC=\"$image_url\" WIDTH=\"450\" HEIGHT=\"500\" ALT=\"Overzichtskaart\" TITLE=\"Overzichtskaart\" ONCLICK=\"form.submit()\">";
			if ( $spiegel == "gemeente" ):
				$map_html .= "<INPUT TYPE=\"hidden\" NAME=\"spiegel\" VALUE=\"overzicht\">";
				$map_html .= "<INPUT TYPE=\"hidden\" NAME=\"choice\" VALUE=\"totaalkaart\">";
			else:
				$map_html .= "<INPUT TYPE=\"hidden\" NAME=\"spiegel\" VALUE=\"$spiegel\">";
				$map_html .= "<INPUT TYPE=\"hidden\" NAME=\"choice\" VALUE=\"$choice\">";
			endif;
			$map_html .= "</FORM>\n";
			$map_html .= "</TD>\n";
			// zoom map 
				...
				$map_html .= "<TABLE WIDTH=\"100%\" BORDER=\"0\">\n";
					...
					$map_html .= "<TR>\n";
						$map_html .= "<TD>\n";
							...
							$map_html .= "<IMG SRC=\"$image_zoom_url\" ALT=\"$kaart_title - $zoom_map_title\" TITLE=\"$kaart_title - $zoom_map_title\" ISMAP USEMAP=\"#map1\" BORDER=\"1\" WIDTH=\"300\">";
							$handle = fopen("/tmp/" . $image_zoom_imagemap_url, "r");
							$contents = fread($handle, filesize("/tmp" . $image_zoom_imagemap_url));
							fclose($handle);
							// there's a funny thing here if choice = some number. We need to undo that situation. It
								// happens when you come in via the dropdown menu.
								switch ( $choice ):
									case "kl_20032005":
									case "rg_20032005":
									case "so_20032005":
									...
									break;
									default:
										$this_choice = "totaalkaart";
									break;
								endswitch;
							$contents = str_replace ( "href='javascript:Clicked();' ", "", $contents );
							$contents = preg_replace ( "/href='javascript:Clicked\((.*)\);'/", "href=\"index.php?spiegel=overzicht&amp;choice=$this_choice&amp;gemeente_naam=$1\" onmouseover=\"return escape('$1')\" ", $contents );
							foreach ($gemeentes as $gemeente_name => $t ){
								$contents = str_replace ( "escape('$gemeente_name", "escape('$gemeente_name " . sprintf( "%.2f", $t["score_procent"] ) . "%", $contents );
							}
							$html_out .= $contents;
						$map_html .= "</TD>\n";
					$map_html .= "</TR>\n";
